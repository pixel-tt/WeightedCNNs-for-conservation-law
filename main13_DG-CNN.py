...
UUr=torch.load('./UU_real')
dx=(x_ran[1]-x_ran[0])/800
x_s=np.linspace(x_ran[0],x_ran[1],800+1)
x=np.concatenate([nodes.reshape(-1,1)*dx/deg+i-dx/deg for i in x_s[1:]],0)
x.sort(0)
index=[i for i in range(num*800) if i%num!=num-1]
func=interpolate.interp1d(x[index,0],UUr[-1][index,:].numpy(),kind='cubic',axis=0,
                          fill_value='extrapolate')
err=[]
for n_cell in [25,50,100,200,400]:
    dx=(x_ran[1]-x_ran[0])/n_cell
    x_s=np.linspace(x_ran[0],x_ran[1],n_cell+1)
    xx=np.concatenate([nodes.reshape(-1,1)*dx/deg+i-dx/deg for i in x_s[1:]],0)[:,0]
    Average1=np.zeros([(num)*n_cell,(num)*n_cell])
    for i in range(n_cell):
        for j in range(num):
            Average1[num*i+j,num*i:num*i+num]=int_f/2
    Average1=torch.from_numpy(Average1)
    UU=torch.load('./UU_{}'.format(n_cell))
    err.append((Average1@(UU[-1]-torch.tensor(func(xx))).abs()).sum(0)/num*dx)

    

#%% order computing(1)
def f(x):
    return torch.sin(torch.pi*4*x)

err1=[]
for n_cell in [25,50,100,200,400,800]:
    dx=(x_ran[1]-x_ran[0])/n_cell
    num=deg+1
    x_s=np.linspace(x_ran[0],x_ran[1],n_cell+1)
    nodes=gLLNodesAndWeights(num)[0]
    ...
    fx=f(x).reshape(-1,1,num*n_cell)
    NN2.check=1
    fx_rec=NN2(fx).detach()
    err1.append(torch.einsum('ij,lmj->lmi',Average1,(fx_rec-fx).abs()).sum()/num*dx)
